...
def decrypt_image(image_path, original_msg_length):
    ...
    # Dictionary to map ASCII values back to characters
    ...
    for i in range(original_msg_length):
        ...
        if char_value in ascii_to_char:
            message += ascii_to_char[char_value]
        else:
            print(f"⚠️ Warning: Unexpected pixel value at [{n}, {m}, {z}] = {char_value}")
            return None
